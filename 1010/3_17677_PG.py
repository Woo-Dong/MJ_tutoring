...
def solution(str1, str2):
    ...
    for i in range(len(str1)-1):
        tmp_word = str1[i:i+2]
        tmp_word = parsing(tmp_word)
        if tmp_word:
            str1_set.append(tmp_word)

    for i in range(len(str2)-1):
        tmp_word = str2[i:i+2]
        tmp_word = parsing(tmp_word)
        if tmp_word:
            str2_set.append(tmp_word)

    answer = 0
    for elem in str1_set:
        if elem in str2_set:
            answer += 1
            str2_set.remove(elem)
    
    total_set_len = len(str1_set) + len(str2_set)

    if total_set_len:
        answer = answer / total_set_len
    else:
        answer = 1
